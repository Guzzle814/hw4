...
#include <cstdlib>
#include <utility>
#include <stack> //Include for clear()
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    {
        Node<Key, Value>* thePred = predecessor(node);
        nodeSwap(node,thePred);
    }
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    Node<Key, Value>* next = current->getLeft();
    while(true)
    {
...
int BinarySearchTree<Key, Value>::balanceHelper(Node<Key, Value>* node) const
{
    ...
    if(balanceHelper(node->getLeft()) == -1)
    {
        return -1;
    }
    else
    {
        leftHeight = balanceHelper(node->getLeft());
    }

    if(balanceHelper(node->getRight()) == -1)
    {
        return -1;
    }
    else
    {
        rightHeight = balanceHelper(node->getRight());
    }
